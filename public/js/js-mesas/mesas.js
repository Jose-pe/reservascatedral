...
function renderFloorplan() {
...
} else if (table.shape === 'large') {
    tableEl.classList.add('rounded-pill');
} else {
    tableEl.classList.add('rounded-4');
        ...
        if (table.capacity === 1) {
            ...
            sizeStyle = { width: '90px', height: '55px', fontSize: '0.7rem' };
        }else if (table.activeZone === 'cava' && table.capacity === 2) {
            sizeStyle = { width: '150px', height: '150px', fontSize: '0.7rem' };
        }else if (table.capacity === 4) {
            sizeStyle = { width: '115px', height: '70px', fontSize: '0.85rem' };
        }else if (table.capacity === 5) {
            sizeStyle = { width: '125px', height: '70px', fontSize: '0.85rem' };
        }else if (table.capacity === 6) {
            sizeStyle = { width: '140px', height: '65px', fontSize: '0.85rem' };
        } else if (table.capacity === 7) {
            sizeStyle = { width: '150px', height: '110px', fontSize: '1rem' };
        }else if (table.capacity === 66) {
            sizeStyle = { width: '100px', height: '180px', fontSize: '1rem' };
        }

        
        if (table.shape === 'large') {
    const altoActual = parseInt(sizeStyle.height);
    const anchoActual = parseInt(sizeStyle.width);
    
    // Si el ancho es mayor que el alto, invertimos las dimensiones para que quede vertical
    if (anchoActual > altoActual) {
        sizeStyle.width = `${altoActual}px`;
        sizeStyle.height = `${anchoActual}px`;
    }
